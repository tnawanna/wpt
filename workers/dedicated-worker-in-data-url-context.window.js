...
promise_test(async t => {
  ...
  // URL dedicated worker, and posts the result to the parent frame.
  const frameCode = `
      <script>
...
promise_test(async t => {
  ...
  // This code will be executed as a data URL dedicated worker. The worker tries
...
promise_test(async t => {
  // This code will be executed as a data URL dedicated worker. The worker
  // creates a nested data URL dedicated worker, and posts the result to the
  // parent frame.
  const workerCode = `
      const worker = new Worker('data:${mimeType},postMessage("PASS");');
